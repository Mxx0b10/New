...
function RadioOption({ label, sublabel, selected, onClick, multi }) {
  return (
    <div
      ...
      onClick={onClick}
      style={{ flex: 1, width: '100%', height: 'auto', minHeight: '96px', boxSizing: 'border-box' }}
    >
      <div style={{ display: 'flex', alignItems: 'start', gap: 4, width: '100%', justifyContent: 'space-between' }}>
        <span style={{
        ...
        <div style={{ position: 'relative', flexShrink: 0, width: 16, height: 16 }}>
          <div className="radio-dot-outer" style={{
            width: 16, height: 16, borderRadius: multi ? '4px' : '50%',
            backgroundColor: selected ? (multi ? '#1A1A1A' : '#FFFFFF') : '#FAFAFA',
            border: `0.5px solid ${selected ? (multi ? '#1A1A1A' : '#636363') : '#929292'}`,
            boxShadow: '0 0 2px 1px #0000000D inset',
      ...
      {sublabel && (
        <div style={{ color: '#8F8F8F', fontFamily: '"Helvetica Neue", Helvetica, system-ui, sans-serif', fontSize: 12, letterSpacing: '0.03em', lineHeight: '16px', marginTop: 8 }}>
          {sublabel}
        </div>
      )}
    </div>
...
function StageProgressBar({ progress = 0 }) {
  const lenses = LENSES.map(l => l.name);
  return (
...
function JourneyStatus({ answers, currentLens, activeLensIndex, onLensClick, isLensUnlocked, onFinishDiagnostic }) {
  const answeredCount = Object.keys(answers).length;
  const progressPercent = Math.round((answeredCount / TOTAL_QUESTIONS) * 100);

  const answeredInCurrentLens = currentLens.questions.filter((q) => answers[q.id] !== undefined).length;
  const totalInCurrentLens = currentLens.questions.length;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {LENSES.map((lens, i) => {
          const isActive = i === activeLensIndex;
          const answeredInIter = lens.questions.filter((q) => answers[q.id] !== undefined).length;
          const totalInIter = lens.questions.length;
          const unlocked = isLensUnlocked(i);

          if (isActive) {
            return (
              <div key={lens.id} style={{
                backgroundColor: lens.activeBg, border: `1px solid ${lens.activeBorder}`,
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                  ...
                  <span style={{
                    color: lens.activeBorder, fontFamily: '"Helvetica Neue", Helvetica, system-ui, sans-serif',
            ...
            return (
              <div key={lens.id}
                className="lens-item-inactive"
                onClick={() => { if (unlocked) onLensClick(i); }}
                style={{
                  backgroundColor: '#FFFFFF', border: '1px solid #E8E8E8',
                  borderRadius: 8, padding: '14px',
...
export default function DiagnosticPage() {
  ...
  const currentLens = LENSES[activeLensIndex];

  const isLensUnlocked = (index) => {
    if (index === 0) return true;
    for (let i = 0; i < index; i++) {
      const lens = LENSES[i];
      const answeredInLens = lens.questions.filter((q) => {
        const ans = answers[q.id];
        return ans !== undefined && (!q.multi || ans.length > 0);
      }).length;
      if (answeredInLens < lens.questions.length) {
        return false;
      }
    }
    return true;
  ...
  const currentQuestion = currentLens.questions[activeQuestionIndex];

  // Helper handling multi vs single
  ...
  const handleNext = () => {
    if (activeQuestionIndex < currentLens.questions.length - 1) {
  ...
  const handleNextLens = () => {
    if (activeLensIndex < LENSES.length - 1) {
      setActiveLensIndex(activeLensIndex + 1);
  ...
  const handlePrevLens = () => {
    if (activeLensIndex > 0) {
      const prevLens = LENSES[activeLensIndex - 1];
      setActiveLensIndex(activeLensIndex - 1);
      setActiveQuestionIndex(prevLens.questions.length - 1);
    }
  };
    ...
    return (
      <div style={{ padding: 48, minHeight: '100vh', backgroundColor: '#F0F0F0', fontFamily: '"Helvetica Neue", Helvetica, system-ui, sans-serif' }}>
         <div style={{ maxWidth: 800, margin: '0 auto', backgroundColor: '#FFF', padding: 48, borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
           <h1 style={{ fontSize: 32, color: '#1A1A1A', fontWeight: 700, marginBottom: 16 }}>Diagnostic Report Complete</h1>
           <p style={{ fontSize: 16, color: '#5F5F5F', lineHeight: '24px' }}>Based on your inputs across the 4 lenses, your Gulf Exposure & India Readiness footprint has been calculated. Your organization shows strong transfer potential in certain operational segments, but requires immediate alignment on risk framing and target audience segmentation.</p>

           <div style={{ marginTop: 32, padding: 24, backgroundColor: '#FAFAFA', border: '1px solid #EAEAEA', borderRadius: 8 }}>
              <div style={{ color: '#1A1A1A', fontWeight: 700, fontSize: 18, marginBottom: 12 }}>Initial Guidance</div>
              <ul style={{ color: '#3C3C3C', margin: 0, paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
                <li>Accelerate IP restructuring regardless of Gulf timeline.</li>
                <li>Conduct independent target persona validation separated from your current Gulf diaspora audience.</li>
                <li>Refine channel prioritization based on defined budget limitations.</li>
              </ul>
           </div>

           <button style={{ marginTop: 32, padding: '12px 24px', backgroundColor: '#1A1A1A', color: '#FFF', borderRadius: 8, cursor: 'pointer', border: 'none', fontWeight: 600 }} onClick={() => window.location.reload()}>Return to Dashboard</button>
         </div>
      </div>
    );
  }
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span style={{
                  color: '#B2B2B2', fontFamily: '"Helvetica Neue", Helvetica, system-ui, sans-serif',
                  fontSize: 12, letterSpacing: '0.02em', lineHeight: '16px', whiteSpace: 'pre',
                }}>Dashboard  /</span>
                <span style={{
                  color: '#3C3C3C', fontFamily: '"Helvetica Neue", Helvetica, system-ui, sans-serif',
                  fontSize: 12, letterSpacing: '0.02em', lineHeight: '16px',
                }}>Gulf Disruption Rebalancing</span>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 32, width: '100%' }}>
                  ...
                  {LENSES.map((lens, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`tab-btn`}
                        style={{
                          backgroundColor: activeLensIndex === i ? lens.color : '#FFFFFF',
                          borderColor: activeLensIndex === i ? lens.borderColor : '#9A9A9A',
                          color: activeLensIndex === i ? '#FFFFFF' : '#353535',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
                <h1 style={{
                  color: '#1F1F1F', fontFamily: '"Helvetica Neue", Helvetica, system-ui, sans-serif',
                  fontSize: 24, fontWeight: 700, letterSpacing: '0.02em', lineHeight: '24px',
                }}>
                  {currentLens.name} — {currentLens.title}
                </h1>
                <p style={{
                  color: '#3C3C3C', fontFamily: '"Helvetica Neue", Helvetica, system-ui, sans-serif',
                  fontSize: 16, letterSpacing: '0.02em', lineHeight: '20px', width: '100%', maxWidth: '800px'
                }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
                <div style={{
                  ...
                  <div style={{ width: '100%' }}>
                    <div style={{
                      ...
                      boxShadow: '0 2px 3px #E0E0E06B, 0 0 0 1px #E0E0E0',
                      padding: '24px 32px',
                      display: 'flex', flexDirection: 'column', gap: 24,
                      overflow: 'clip',
                    }}>
                      {/* Question text + insight */}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                        <div style={{ display: 'flex', gap: 4, justifyContent: 'space-between' }}>
                          <p style={{
                            color: '#1F1F1F', fontFamily: '"Helvetica Neue", Helvetica, system-ui, sans-serif',
                            fontSize: 18, letterSpacing: '0.01em', lineHeight: '22px',
                          }}>
                      ...
                      {/* Radio/Chips options container */}
                      <div style={{ 
                        display: currentQuestion.layout === 'chips' ? 'flex' : 'grid', 
                        flexWrap: currentQuestion.layout === 'chips' ? 'wrap' : 'nowrap',
                        gridTemplateColumns: currentQuestion.layout === 'chips' ? 'none' : 'repeat(2, 1fr)', 
                        gap: 12, width: '100%' 
                      }}>
                          ...
                          if (currentQuestion.layout === 'chips') {
                            return (
                              <button
                                ...
                                style={{
                                  backgroundColor: selected ? (currentLens.color || '#1A1A1A') : '#FFFFFF',
                                  color: selected ? '#FFFFFF' : '#1A1A1A',
                                  border: `1px solid ${selected ? (currentLens.color || '#1A1A1A') : '#EAEAEA'}`,
                ...
                <div style={{
                  ...
                  gap: 8, paddingBlock: 4,
                }}>
                  <button className="nav-btn nav-btn-secondary" onClick={handleBack} disabled={activeLensIndex === 0 && activeQuestionIndex === 0}>Back</button>
                  <button className="nav-btn nav-btn-white" 
                    ...
                    style={{
                      opacity: (answers[currentQuestion.id] !== undefined && (!currentQuestion.multi || answers[currentQuestion.id].length > 0)) ? 1 : 0.5,
                      cursor: (answers[currentQuestion.id] !== undefined && (!currentQuestion.multi || answers[currentQuestion.id].length > 0)) ? 'pointer' : 'not-allowed'
                    }}
                  >Next</button>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', flex: '1 1 300px', maxWidth: 333, margin: '0 auto' }}>
              <JourneyStatus 
               ...
               onLensClick={(idx) => { setActiveLensIndex(idx); setActiveQuestionIndex(0); }}
               isLensUnlocked={isLensUnlocked}
